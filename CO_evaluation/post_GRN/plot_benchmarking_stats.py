...
from GRN.imports import BENCHMARK_CO_DIR
...
def plot_both(df):
    ...
    # Plot the bars
    bar1 = plt.bar(index, df['ratio_GT'], bar_width, color='skyblue', label='ratio_GT')
    bar2 = plt.bar([i + bar_width for i in index], df['ratio_links'], bar_width, color='coral', label='ratio_links')
